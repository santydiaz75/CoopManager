...
SET DB_PASS=
SET MY=no
SET /P DB_USER=[ INGRESE USUARIO MYSQL:] 
SET /P DB_PASS=[ INGRESE CONTRASENA:] 
rem | -------------------------------------------------------------------------------------------------------------------------------------
rem | Chequeo Acceso
...
echo --------------------------------------------------------------------------
SET DB_NAME=
SET /P DB_NAME=[ INGRESE NOMBRE DE LA BASE DE DATOS, (ALL = TODAS LAS BDS):] 
rem | -------------------------------------------------------------------------------------------------------------------------------------
rem | Chequeo DB
...
SET PARAMS=
SET /P PARAMS=[ PARAMETROS ADICIONALES? (enter para none):] 
GOTO :set_check
rem | -------------------------------------------------------------------------------------------------------------------------------------
rem | Chequeo y Optimización
:set_check
SET DO_CHECK=1
SET /P DO_CHECK=[ DESEA REALIZAR CHEQUEO Y OPTIMIZACION? (1/0):] 
GOTO :mysql2
rem | -------------------------------------------------------------------------------------------------------------------------------------
:mysql2
...
if %DB_NAME%==ALL SET DB_NAME=-A
mysqldump -l --result-file=%FILE% --log-error=%LOG% %PARAMS% -u %DB_USER% -p%DB_PASS% %DB_NAME%
if %DB_NAME%==-A SET DB_NAME=ALL
echo.
...
echo --------------------------------------------------------------------------
echo.
echo   Programa Finalizado. Cualquier tecla para salir...
echo.
echo --------------------------------------------------------------------------
echo.
echo                                                      KNOX-IT.BLOGSPOT.COM
echo                                                      KNOX-IT.BLOGSPOT.COM
echo                                                      KNOX-IT.BLOGSPOT.COM
echo                                                      KNOX-IT.BLOGSPOT.COM
echo                                                      KNOX-IT.BLOGSPOT.COM
