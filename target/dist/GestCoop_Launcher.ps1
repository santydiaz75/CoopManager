...
# Funci�n para escribir al log simplificada para ps2exe
...
try {
    ...
    # Configurar Java 21 para esta sesi�n
    ...
    if ($process -ne $null) {
        ...
        if ($process.HasExited) {
            ...
            Write-AppLog "Proceso termin�. C�digo: $exitCode" "ERROR"
            Write-AppLog "STDOUT: $stdout" "ERROR"
            Write-AppLog "STDERR: $stderr" "ERROR"
            
            $errorMsg = "GestCoop se cerr� inesperadamente.`nC�digo: $exitCode"
            if ($stderr) { $errorMsg += "`nError: $stderr" }
            Show-Error $errorMsg
            exit 1
        }
        
        Write-AppLog "GestCoop ejecut�ndose correctamente" "INFO"
        exit 0
    } else {
